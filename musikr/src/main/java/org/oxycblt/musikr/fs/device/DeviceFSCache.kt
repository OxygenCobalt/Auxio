 ...
 *
 * This cache is based on the concept that a directory's modification date changes when its direct
 * contents change (files added, removed, renamed). We can avoid expensive children queries by
 * caching directory contents and validating them against the directory's modification timestamp.
 */
internal class DeviceFSCache(private val context: Context) {
    ...
    fun getDirectoryInfo(uri: Uri): DirectoryInfo? {
        return contentResolver.useQuery(
            ...
            arrayOf(
                ...
                DocumentsContract.Document.COLUMN_LAST_MODIFIED)) { cursor ->
    ...
    fun getDirectoryChildren(uri: Uri): List<DirectoryChild>? {
        ...
        return contentResolver.useQuery(childrenUri, CHILDREN_PROJECTION) { cursor ->
    ...
    /**
     * Validates the cached data against directory's current state. If directory's modification time
     * has changed, cache is updated.
     *
    ...
    fun validateDirectoryCache(uri: Uri): Boolean {
        ...
        cache[uriString] = CachedDirectory(modifiedDate = dirInfo.lastModified, children = children)

        return false
    }

    /**
     * Gets cached directory children without validation. Use validateDirectoryCache() first to
    ...
    /** Cleans up the cache by removing entries that weren't accessed in the current traversal. */
    fun cleanupCache() {
...
data class DirectoryInfo(val name: String, val lastModified: Long)
...
@Serializable
data class CachedDirectory(val modifiedDate: Long, val children: List<DirectoryChild>)

/** Structure for serializing/deserializing the entire cache. */
@Serializable
data class CacheData(val schemaVersion: String, val entries: Map<String, CachedDirectory>)
