...
quick(){ check SUBMODULE_BLOCKER prepare bash "$ROOT/scripts/prepare-ci-environment.sh"; check SUBMODULE_BLOCKER submodules bash "$ROOT/scripts/check-submodules.sh"; check REAL_BUILD_FAILURE shell-syntax bash -lc 'find "$0/scripts" "$0/docs/codex" -type f -name "*.sh" -print -exec sh -n {} \;' "$ROOT"; check JAVA_GRADLE_COMPAT_BLOCKER gradle-version with_timeout 600 bash -lc 'cd "$0" && ./gradlew --no-daemon --version' "$ROOT"; check ANDROID_SDK_BLOCKER gradle-help with_timeout 900 bash -lc 'cd "$0" && ./gradlew --no-daemon --stacktrace help' "$ROOT"; }
...
docs_mode(){ check OK ts18-source-led-doc test -f "$ROOT/docs/TS18_SOURCE_LED_INTEGRATION_STRATEGY.md"; check OK ts18-runbook test -f "$ROOT/docs/TS18_VALIDATION_RUNBOOK.md"; }
