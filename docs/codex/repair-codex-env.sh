...
if [[ "${CODEX_UPDATE_SUBMODULE_REMOTES:-0}" = "1" ]]; then
  run_optional "Submodule remote update (opt-in)" bash -lc 'cd "$0" && git submodule update --remote --recursive' "$ROOT"
else
  skip "Submodule --remote disabled by default"
fi
...
if [[ "${CODEX_CLEAN:-0}" = "1" ]]; then
  run_optional "Clean gradle locks (opt-in)" bash -lc 'find "$HOME/.gradle" -type f -name "*.lock" -delete'
else
  skip "Gradle lock cleanup disabled by default"
fi
write_summary
