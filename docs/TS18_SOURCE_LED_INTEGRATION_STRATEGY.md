...
|---|---|---|---|---|---|---|---|---|---|---|
| Local DoFun/Variety compatibility bundle | `docs/DOFUN_VARIETY_COMPATIBILITY.md`; `docs/topway-dofun-variety/reference/dofun-variety/` | local extracted APK evidence | `com.dofun.variety` manifest/assets; `apps_match_config` fixed music hotseat entries; `apps_config` stock music mappings; string hits for Cardoor media service surfaces | DoFun/Variety fixed local-music matching prefers `com.tw.media`/`com.tw.music` with `com.tw.music.MusicActivity` and labels `音乐,音樂,Music` | A dedicated, separate Topway/DoFun APK identity can satisfy the fixed launcher slot while normal Auxio identity remains intact | Private `cn.cardoor.libs.media.RemoteMediaService` binder contract or full widget runtime behavior | Use source-defined `topwayTwMusic` flavour, Android media/session/browser, and isolated Topway bridge; keep private Cardoor service unimplemented pending contract recovery | Observed | Directly reusable requirement for package/class/label matching; Requires TS18 runtime validation for widget behavior | Extracted APK artefacts are evidence only; do not copy protected code, smali, or implement unknown private binders |
